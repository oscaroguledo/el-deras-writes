...
export function Header() {
  ...
  return <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <form onSubmit={handleSearch} className="relative">
              <input type="text" placeholder="Search articles..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-64 py-1 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-gray-500" />
            ...
            <nav className="flex space-x-8">
              ...
              <Link to="/?category=Technology" className="text-gray-600 hover:text-gray-900 px-3 py-2 text-sm font-medium">
                Technology
              </Link>
            </nav>
          </div>
      ...
      <div className={`md:hidden fixed top-0 right-0 bottom-0 w-64 bg-white shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="px-4 py-6">
          <form onSubmit={handleSearch} className="relative mb-6">
            <input type="text" placeholder="Search articles..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full py-2 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-gray-500" />
          ...
          <nav className="space-y-1">
            ...
            <Link to="/?category=Technology" className="block px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md" onClick={() => setIsMenuOpen(false)}>
              Technology
            </Link>
            <Link to="/?category=Design" className="block px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md" onClick={() => setIsMenuOpen(false)}>
              Design
            </Link>
            <Link to="/?category=Lifestyle" className="block px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-md" onClick={() => setIsMenuOpen(false)}>
              Lifestyle
            </Link>
          </nav>
        </div>
